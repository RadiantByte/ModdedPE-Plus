...
import com.radiantbyte.moddedpe.plus.mcpelauncher.ui.pages.PreloadingPage
import com.radiantbyte.moddedpe.plus.mcpelauncher.ui.theme.ModdedTheme
import com.radiantbyte.moddedpe.plus.R
import com.radiantbyte.moddedpe.plus.pesdk.PreloadException
import com.radiantbyte.moddedpe.plus.pesdk.Preloader
import com.radiantbyte.moddedpe.plus.pesdk.nmod.NMod
import com.radiantbyte.moddedpe.plus.pesdk.somod.SoModManager
import com.radiantbyte.moddedpe.plus.pesdk.somod.SoModNativeLoader
    ...
    private val preloadUIHandler = object : Handler(Looper.getMainLooper()) {
        override fun handleMessage(msg: Message) {
            ...
            when (msg.what) {
                ...
                MSG_START_MINECRAFT -> {
                    val intent = Intent(this@ComposePreloadActivity, com.radiantbyte.moddedpe.plus.mcpelauncher.activities.MinecraftActivity::class.java)
                    ...
                    object : Preloader.PreloadListener() {
                        override fun onStart() {
                            ...
                            if (Preferences.isSafeMode) {
                                writeNewText(
                                    getString(
                                        R.string.preloading_initing_info_safe_mode,
                                        ModdedPEApplication.mPESdk.minecraftInfo.minecraftVersionName
                                    )
                                )
                            } else {
                                writeNewText(
                                    getString(
                                        R.string.preloading_initing_info,
                                        ModdedPEApplication.mPESdk.nModAPI.versionName,
                                        ModdedPEApplication.mPESdk.minecraftInfo.minecraftVersionName
                                    )
                                )
                            }
                        ...
                        override fun onNModLoaded(nmod: NMod) {
                            writeNewText(
                                getString(
                                    R.string.preloading_nmod_loaded,
                                    nmod.packageName
                                )
                            )
                        }
                        ...
                        override fun onFailedLoadingNMod(nmod: NMod) {
                            writeNewText(
                                getString(
                                    R.string.preloading_nmod_loaded_failed,
                                    nmod.packageName
                                )
                            )
